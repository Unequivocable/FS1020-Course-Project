...
//NOTE--after writing the checkProps I discovered it was also checking that the field exists, so this check was redundant
...
const checkProps = (req) => {
    ...
    if (containsProp.includes("email") && (req.body.email == null || !/.+\@.+\..+/.test(req.body.email) || req.body.email == "" )){
        invalidProp.push("email")
    }

    if (containsProp.includes("password") && (req.body.password == null || !/^\w{8,}$/.test(req.body.password) || req.body.password == "" )){
        invalidProp.push("password")
    }

    if (containsProp.includes("phonenumber") && (req.body.phonenumber == null || !/^\d{10}$/.test(req.body.phonenumber) || req.body.phonenumber == "" )){
